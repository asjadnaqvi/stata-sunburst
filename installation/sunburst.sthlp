...
{* 24December2022}{...}
{hi:help sunburst}{...}
{right:{browse "https://github.com/asjadnaqvi/stata-sunburst":sunburst v1.0 (GitHub)}}
...
{cmd:sunburst} {it:value} {ifin}, {cmd:by}({it:variables}) 
                {cmd:[} {cmdab:rad:ius}({it:numlist}) {cmd:step}({it:num}) {cmd:palette}({it:str}) {cmd:colorby}({it:option}) {cmd:share} 
                  {cmdab:thresh:old}({it:cutoff}) {cmdab:valcond:ition}({it:str}) {cmd:format}({it:str}) {cmdab:lw:idth}({it:list})  
...
{p2coldent : {opt sunburst value, by(variables)}}The command requires a numerical {it:value} variable, plotted over a sequence of heirarchical variables defined by {opt by()}.
...
The first layer defined as the first option in {opt by()} will deterime the colors. The next set of layers will take on this color with slightly lower fill intensities.{p_end}
...
gradient fill to the {it:last} layer. Where the gradient is interpolated between the base color and a light grey tone. This can give a nice touch if the 
categories are sufficiently evenly distributed. Note that this option will use the layer immediately below to determine the gradient categories.
...
{p2coldent : {opt valcond:ition(str)}}The condition for showing value labels. For example, if we only want to display categories with a greater than a value of 100, we can specify
{opt valcond(>100)}. If the {opt share} is used, then please specify the share threshold value. Default is {opt valcond(>=0)}.{p_end}
